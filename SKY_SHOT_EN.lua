...
function ydks()
	...
	local time=os.clock()
	while (os.clock() - time) < 20 do -- Ejecuta la función durante 20 segundos
...
posits = {
    ...
    {map='Prairie_Island',name='Island vortex',x=-385.6,y=1.4,z=160.1},
...
function settingsmenu()
    local useautoburn = settings.useautoburn and sign(settings.useautoburn) or "false"
    local smenu = gg.choice({
        "Distance breackwall: "..settings.wdistance,
        "Set Delay for RapidFarm: "..settings.delay
    }, nil, "settings:")
...
function FarmingMod()
	...
	local choice = gg.choice(options, nil, "CHOICE MODS")
	...
	elseif choice == 4 then
		local newVal1 = suic1 == "on" and 1.19212586E-7 or -9010873890000
		setvalue(so + bc, 16, newVal1)
		suic1 = suic1 == "on" and "off" or "on"
		local msg1 = suic1 == "on" and "Enabled - Burn Candle" or "Disabled - Burn Candle"
		gg.toast(msg1)
			
		local newVal2 = suic2 == "on" and bp_v or 1.41233962E-13
		setvalue(so + bp, 16, newVal2)
		suic2 = suic2 == "on" and "off" or "on"
		local msg2 = suic2 == "on" and "Enabled - Burn Plant" or "Disabled - Burn Plant"
		gg.toast(msg2)	  
	end
end
...
function WorldMods()
	...
	local choice = gg.choice(options, nil, "Wordl MODS")
	if not choice then return end
...
function node()
	...
	if suiu2 == "on" then
	    ...
	    gg.clearResults()
	    gg.toast("🤼 on")
		gg.sleep(500) -- Agregar un retraso de 500 ms
	else
		gg.setRanges(gg.REGION_CODE_APP)
...
function wingpowers()
	...
	local choice = gg.choice(options, nil, 'WARNING, DON\'T TRAP YOURSELF')
	if choice == nil then
		noselect()
	else
		if choice == 1 then
...
function start()
	cr_mode = true
	menu = gg.choice({
		"[⬆️] Brackwall"..settings.wdistance,
		"[🕯️] Candles Run",
		"[🌎] Go to",
		"[🌎] Word Mods",
		"[💫] Simple magics",
