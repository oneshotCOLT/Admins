...
function vcheck()
	if gameinfo.packageName ~= scriptv.process then
		gg.alert('[Error] ¡Ha seleccionado un proceso incorrecto!\ngame : ' .. gameinfo.packageName)
		os.exit()
	end
	
	if tonumber(gameinfo.versionCode) < scriptv.version then
		gg.alert('[Error] El juego se actualizo! \ngame : ' .. tonumber(gameinfo.versionCode) .. '\nscript : ' .. scriptv.version)
	end

	if tonumber(gameinfo.versionCode) > scriptv.version then
		gg.alert('[Error] El guion necesita actualizacion! \ngame : ' .. tonumber(gameinfo.versionCode) .. '\nscript : ' .. scriptv.version)
	end
end
...
magicsid = {
	{'❌nada',0,0},
	{'💫Pequeño',1692428656,0},
	{'💫Enano',2142718166,0},
	{'💫Grande',891098028,0},
	{'💫Grandote',-1879316162,0},
	{'💫Resplandor',1097748727,0},
	{'💫Recarga',1750685908,0},
	{'💫Flotar',1860519737,0},
	{'💫Anti Dragon',1067647386,0},
	{'💫Anti Oscuridad',383062578,0},
	{'💫Anti rayo',-1463943689,0},
	{'💫Mucha luz de Velas',-1727483534,0},
	{'💞Bendicion de vuelo',-1362469224,0},
	{'💞Amistad codo a codo',1405645877,0},
	{'💞Abrazo de oso',1677246236,0},
	{'💞Warp de amistad',998754260,0},
	{'🙀llamado de cangrejo',1725047129,0},
	{'🙀llamado de pez',-957441587,0},
	{'🙀Llamado de manta',1814753890,0},
	{'🧸Linterna',1319710173,0},
	{'🧸Mesa',256924066,0},
	{'🧸Arpa',-1001892262,0},
	{'🧸Swing',1064080243,0},
	{'🧸Balancin',-2095749161,0},
	{'🧸Mesa de Te',10058409,0},
	{'🧸Paraguas Transparente',1167393926,0},
	{'👗Capa capullo verde',2079599063,1},
	{'👗Capa tgc',540856305,1},
	{'👗Baticapa',625581156,1},
	{'👗Capa spider',930203946,1},
	{'👗Capa copo de nieve',-784831205,1},
	{'👗Capa navidad',1306675982,1},
	{'👗Capa blanca de ave',-1623262339,1},
	{'👗Capa petalo',-6043825,1},
	{'👗Bufanda principito',1375571404,1},
	{'👗Capa Primavera', -445538750,1},
	{'👗Capa sakura',162066154,1},
	{'👗Capa oceano',329684861,1},
	{'👗Capa de terciopelo rojo de la temporada de ensueño',-308941587,1},
	{'👗Capa de graduación de la temporada de ensueño',-1822337532,1},
	{'👗capa arcoiris', -195929339,1},
...
function FastCR(name)
	...
	for i, v in ipairs(points) do
		...
		if m ~= v.map then
			gg.toast("Por favor, enter "..name.." o vete a home e intenta nuevamente.")
			return
		end
		break
	end
...
function PointsEnd(map)
	...
	if n ~= nil then
		while true do
			gg.setVisible(false)
			action = gg.choice(mmenu, nil, "Estamos en: "..get_map_name().."\n (Teleport solo te lleva a ti)")
		...
		gg.toast("No se encontró el siguiente mapa")
		return
	end
end
...
function settingsmenu()
	local smenu = gg.choice({
		"Distancia de rompemuros: "..settings.wdistance,
		"Usar Autoquemado en AutoCR: "..sign(settings.useautoburn)
	}, nil, "Settings:")
	...
	if smenu == 1 then
		local wbd = gg.prompt({"Distancia por defecto: "..settings.wdistance}, {[1] = settings.wdistance}, {[1] = "number"})
		if wbd ~= nil then
			settings.wdistance = wbd[1]
		end

		gg.toast("Distancia de rompemuros: "..settings.wdistance)
	elseif smenu == 2 then
		settings.useautoburn = not(settings.useautoburn)
		gg.toast("Usar autoquemado en AutoCR: "..sign(settings.useautoburn))
	end
end
...
function Start()
  cr_mode = true
  menu = {
    "[⬆️] Rompemuros: " .. settings.wdistance,
    "[⏭] Correr velas",
    "-"
  }

  -- Agrega los elementos de la lista al menú principal
  for k,v in ipairs(list) do
    table.insert(menu, v)
  end

  -- Imprime el menú actualizado
  gg.choice(menu, nil, "Creado por Meph:")
  ...
  elseif menu == 2 then
    ...
    -- Encuentra el índice del elemento seleccionado y ajusta el offset
    local index = menu - 3
    if index > 0 then
      local map = get_map()
      ...
      if ppoints ~= nil then
        ...
        place = gg.choice(mp_names, nil, "¿A dónde vamos?")
        ...
        gg.toast("No hay lugares a donde ir.")
      end
    end
  end
end
